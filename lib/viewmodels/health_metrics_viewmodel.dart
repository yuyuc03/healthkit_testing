...
class HealthMetricsViewModel extends ChangeNotifier {
  ...
  Future<void> _loadInitialData() async {
    try {
      for (var metric in _metrics) {
        final storedData = await _databaseHelper.getLatestMetric(metric.type);
        if (storedData != null) {
    ...
    try {
      ...
      if (authorized) {
        ...
        for (var fetchedMetric in fetchedData) {
          final index = _metrics.indexWhere((m) => m.type == fetchedMetric.type);
          if (index != -1) {
            _metrics[index] = fetchedMetric;
            
            // Store in database
            await _databaseHelper.insertHealthMetric(fetchedMetric);
          }
        }
      }
    } catch (e) {
  ...
  Future<List<HealthMetric>> getHistoricalData(HealthDataType type) async {
    try {
      return await _databaseHelper.getHealthMetricsForML(type.name);
    } catch (e) {
